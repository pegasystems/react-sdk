...
export default function AutoComplete(props: AutoCompleteProps) {
  ...
  const { isOnlyField, overrideLabel } = useIsOnlyField(displayOrder as any);
  ...
  function customAssignmentFinished() {
    if (sessionStorage.getItem(`autocompleteValue${name}${caseId}`) !== null) {
      // @ts-ignore
      sessionStorage.setItem(`autocompleteEmptyValue${name}${caseId}`, sessionStorage.getItem(`autocompleteValue${name}${caseId}` || ''));
    }
  }
  ...
  useEffect(() => {
    ...
    PCore.getPubSubUtils().subscribe('CustomAssignmentFinished', customAssignmentFinished, 'customAssignmentFinished');
    ...
    if (!displayMode && listType !== 'associated') {
      getDataPage(datasource, parameters, context).then((results: any) => {
        ...
        results?.forEach(element => {
          ...
          const valTranslated = PCore.getLocaleUtils().getLocaleValue(val, localePath, translationDataPage);
          const obj = {
  ...
  useEffect(() => {
    ...
    const elementUl = document.getElementById(`${formattedPropertyName}__listbox`) as HTMLInputElement;
  ...
  if (hasBeenWrapped && configAlternateDesignSystem?.ShowChangeLink && inprogressStatus === 'Open-InProgress') {
    return (
      <GDSCheckAnswers
        ...
        hideLabel={false}
        emptyValue={sessionStorage.getItem(`autocompleteEmptyValue${name}${caseId}`) as any}
      />
    );
  }
