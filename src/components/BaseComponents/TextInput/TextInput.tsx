...
export default function TextInput(props) {
  ...
  const textInputClassesWithWidth = (width: number) => {
    if (width !== undefined && width !== null) {
      ...
      if (width >= 11 && width <= 20) return `${inputClasses} govuk-input--width-${20}`.trim();
      if (width >= 21) return `${inputClasses}`.trim();
  ...
  return (
    <FormGroup {...props}>
      <input className={textInputClassesWithWidth(maxLength)} {...inputProps} id={id} name={name} onBlur={onBlur} onKeyDown={keyHandler} />
    </FormGroup>
  );
}
