...
export default function Integer(props: IntegerProps) {
  ...
  // Disallow "." and "," since this is an integer field
  function intOnChange(event) {
    const disallowedChars = ['.', ','];
    const attempted = event.target.value;
    if (disallowedChars.includes(attempted.slice(-1))) {
      event.target.value = attempted.slice(0, -1);
    }
  ...
  const inputId = `nm-integer-${testId ?? propName ?? label}`;
