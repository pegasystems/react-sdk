...
export const PegaExtensionsUKGovLayout = (props: layoutProps) => {
  ...
  const handlePreviousClick = () => {
    if (layoutRef.current) {
      ...
      child[current].style.removeProperty('display');
    }
  };
  ...
  const handleNextClick = () => {
    if (layoutRef.current) {
      ...
      if (current !== child.length - 1) {
        ...
        prevRef.current.style.removeProperty('display');
      } else {
        ...
        const actions = document.querySelector('.MuiGrid-root.MuiGrid-container.MuiGrid-spacing-xs-4.css-bc1tov-MuiGrid-root') as HTMLElement;
        if (actions) {
