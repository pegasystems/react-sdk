...
export default function ListViewWrapper(props: ListViewProps) {
  ...
  return (
    <div style={{ padding: 24, background: '#ffffff' }}>
      ...
      {template === 'Gallery' && <Carousel data={items} title={title} showAllItems={showAllItems} referenceDataPage={referenceDataPage} />}
      {open && (
        <Modal onClose={closeModal}>
          <div className='alv-modalShell' role='document'>
            <div className='alv-modalHeader'>
              <div>
                <h3 className='alv-modalTitle'>{title === 'Activity' ? 'All Activities' : title}</h3>
                ...
                <div className='gallery-grid-container'>
                  {items.map((item, idx) => (
                    <div key={item.id ?? idx} className='gallery-grid-item'>
                      <img src={item.ImageURL} alt={item.Carouselheading || 'Carousel Image'} />
