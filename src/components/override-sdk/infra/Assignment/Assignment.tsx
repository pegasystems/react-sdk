    ...
    <>
      {serviceShutteredStatus ? (
        ...
        <div id='Assignment'>
          ...
          {
            // If there is no 'Previous' action button, show a backlink that uses the app context backlink action
            !(arSecondaryButtons?.find((button) => button['name'] === 'Previous')) && appBacklinkAction &&  <Button
              variant='backlink'
              onClick={e => {
                e.target.blur();
                appBacklinkAction();
