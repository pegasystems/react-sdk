...
export default function QuickCreate(props: QuickCreateProps) {
  ...
  if (!mashupCaseType) {
    const caseTypes = (PCore as any).getEnvironmentInfo().environmentInfoObject.pyCaseTypeList;
  ...
  useEffect(() => {
    ...
    if (envInfo?.environmentInfoObject?.pyCaseTypeList) {
      (envInfo.environmentInfoObject.pyCaseTypeList as any).forEach(casetype => {
