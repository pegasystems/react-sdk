...
// Luna form_control-date_picker pattern: native date input, bottom-border only
const StyledInput = styled.input<{ $hasError?: boolean; $readOnly?: boolean }>`
  ...
  /* Hide native browser calendar icon; our SVG overlays it */
...
export default function Date(props: DateProps) {
  ...
  const inputId = `nm-date-${testId ?? propName ?? label}`;
